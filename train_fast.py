"""
Fast async training script for Othello AI.
...
Usage:
    python train_fast.py --steps 100000 --workers 32

GPU Auto-Detection:
    This script automatically adds conda CUDA 12.8 to PATH if needed.
    Your RTX Pro 6000 Blackwell should be detected automatically.
"""
...
def get_progressive_simulations(step: int) -> int:
    """
    Progressive MCTS simulation schedule.
    Early training uses fewer simulations (model is random, deep search wasted).
    Later training uses more simulations (model is strong, deep search valuable).
    """
    if step < 5000:
        return 100
    elif step < 20000:
...
def parse_args():
    parser = argparse.ArgumentParser(description='Fast Async Training for Othello AI')
...
def main():
    ...
    trainer = Trainer(
        model=model,
        mcts=None,  # Not used in async mode
    ...
    if args.num_simulations is None:
        ...
        print(f"  Steps 0-5K:     100 simulations")
        print(f"  Steps 5K-20K:   200 simulations")
        print(f"  Steps 20K-50K:  400 simulations")
        print(f"  Steps 50K+:     800 simulations")
